...
class FPN_(Backbone):
    ...
    def __init__(
        ...
        self.fpn_feature_gen = fpn_feature_gen
    ...
    def forward(self, x):
        ...
        # Reverse feature maps into top-down order (from low to high resolution)
        bottom_up_features = self.bottom_up(x)
        results = self.fpn_feature_gen(bottom_up_features)
        return dict(zip(self._out_features, results))
...
@BACKBONE_REGISTRY.register()
def build_darknet53_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
...
@BACKBONE_REGISTRY.register()
def build_retinanet_resnet_fpn_backbone_test(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
...
def build_retinanet_mobilenetv2_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    in_features = cfg.MODEL.FPN.IN_FEATURES
    out_channels = cfg.MODEL.FPN.OUT_CHANNELS
    in_channels_p6p7 = bottom_up.output_shape()["s5"].channels
    backbone = FPN(
        bottom_up=bottom_up,
        in_features=in_features,
        out_channels=out_channels,
        norm=cfg.MODEL.FPN.NORM,
        top_block=LastLevelP6P7(in_channels_p6p7, out_channels, in_feature="s5"),
        fuse_type=cfg.MODEL.FPN.FUSE_TYPE,
    )
    return backbone

@BACKBONE_REGISTRY.register()
def build_retinanet_darknet53_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
